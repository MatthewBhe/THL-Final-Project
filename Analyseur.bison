...
%type <set> set_expr union_expr intersect_expr diff_expr primary set_expr_list
%type <num> card_expr
...
stmt:
      IDENT ASSIGN set_expr { 
          ...
          sym->set = $3; sym->isNum = 0; sym->defined = 1; 
          printf("%s = ", sym->name); 
          printSet(sym->set); 
          printf("\n"); 
          free($1); 
      }
    | IDENT ASSIGN card_expr { 
    ...
    | card_expr { printf("%d\n", $1); }
    | set_expr '=' set_expr { 
          if(set_equal($1, $3)) printf("true\n"); 
          else printf("false\n"); 
          free($1); free($3); 
      }
    | set_expr IN set_expr { 
          if(set_inclusion($1, $3)) printf("true\n"); 
          else printf("false\n"); 
          free($1); free($3); 
      }
    ;
...
card_expr:
      CARD set_expr { $$ = countBits($2); free($2); }
    ;
...
primary:
    ...
    | '(' set_expr ')' { $$ = $2; }
    | UNION '(' set_expr_list ')' { $$ = $3; }
    ;

set_expr_list:
      set_expr { $$ = $1; }
    | set_expr_list ',' set_expr { 
            unsigned long long* tmp = set_union($1, $3); 
            free($1); free($3); 
            $$ = tmp; 
        }
